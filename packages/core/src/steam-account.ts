...
import type { SteamOwnedGame } from "./types.js";
...
export const detectLocalSteamId = async (): Promise<string | null> => {
  ...
  const candidates = await Promise.all(users
    .filter((entry) => entry.isDirectory() && /^\d+$/.test(entry.name))
    .map(async (entry) => ({
      accountId: entry.name,
      modifiedAt: (await fs.promises.stat(path.join(userdata, entry.name, "config/localconfig.vdf")).catch(() => null))?.mtimeMs ?? 0,
    })));
...
export const fetchOwnedSteamGames = async (
  ...
  if (!/^[A-F0-9]{32}$/i.test(apiKey.trim())) throw new Error("La API key de Steam no es válida");
  if (!/^7656119\d{10}$/.test(steamId)) throw new Error("El SteamID64 no es válido");
  ...
  const response = await fetch(`https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?${query}`, {
    headers: { "x-webapi-key": apiKey.trim() },
  });
  if (!response.ok) throw new Error(`Steam respondió con ${response.status}`);
